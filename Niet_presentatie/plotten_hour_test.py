...
if __name__ == "__main__":
    with open("lattice_results.pkl", "rb") as f:
    ...
    for i, (gen_type, elapsed) in enumerate(zip(['S', 'T', 'T_pairwise', 'T_cop', 'U_+', 'U_-', 'U_two'], time_elapsed)):
        print(f"  {gen_type}: {elapsed} seconds")
    ...
    # Printing for every generator type and dimension the maximum n value
    print("Maximum n values for each generator type and dimension:")
    for gen_type in df['type'].unique():
        for d in df['d'].unique():
            max_n = df[(df['type'] == gen_type) & (df['d'] == d)]['n'].max()
            if pd.notna(max_n):
                print(f"  Generator Type: {gen_type}, Dimension: {d}, Max n: {max_n}")
    ...
    def plot_generator_type(ax, gen_type, x_upper_limit=None, y_upper_limit=None):
        ...
        #Set axis limits based on the maximum for this generator type and the filtered data
    ...
    # Plot each generator type in its own plot with filtered data for each dimension
    for gen_type in types:
        fig, ax = plt.subplots(figsize=(10, 6))
        plot_generator_type(ax, gen_type)
        fig.tight_layout()
        plt.show()
    ...
    plot_all_types(axes2[0], DEFAULT_P_MAX, DEFAULT_P_MAX, title_range='[2,6]')
    plot_all_types(axes2[1], 20.1, 20.1, title_range='[2,9]')
    fig2.tight_layout()
    plt.show()

    # Compute for each generator, for the interval p/q in [2,6], for how large a part the n^{1/d} is the best of all generators.
    # So what percentage of x in [2,6], is the liny y for this generator the highest
    # ==========================================
